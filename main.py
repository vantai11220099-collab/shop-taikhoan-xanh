import os, sqlite3, re, random, string, asyncio
...
async def casso_webhook(request):
    ...
    desc = data['data'][0]['description'].upper()
...
async def main():
    ...
    web_app.router.add_post('/webhook', application.webhook_handler())
    web_app.router.add_post('/casso', casso_webhook)
